...
[<selector>]*
'`{`'
   ['`.name`' <single-quoted styling rule name>]
...
Variables can be defined before styling rule names with this syntax:

`@` <identifier> '`=`' <expression> '`;`'

A variable (`@` <identifier>) can then be used as an expression.
